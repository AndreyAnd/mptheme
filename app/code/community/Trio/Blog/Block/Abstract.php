...
abstract class Trio_Blog_Block_Abstract extends Mage_Core_Block_Template
{
    ...
    protected static $_catUriParam = Trio_Blog_Helper_Data::CATEGORY_URI_PARAM;
    
    protected static $_postUriParam = Trio_Blog_Helper_Data::POST_URI_PARAM;
    
    protected static $_tagUriParam = Trio_Blog_Helper_Data::TAG_URI_PARAM;
    ...
    {
        if (self::$_helper->conf(Trio_Blog_Helper_Data::XML_BOOKMARKS)) {
            return $this->setTemplate('trio_blog/bookmark.phtml')->setPost($post)->renderView();
        }
    }
    ...
    public function getTagsHtml($post)
    {
        if (trim($post->getTags())) {
            return $this->setTemplate('trio_blog/line_tags.phtml')->setPost($post)->renderView();
        }
    }
    ...
    {      
        return '<a class="trio-blog-read-more" href="'.$item->getAddress().'">'.$this->__('Read More').'</a>';
    }
    ...
    {
        return Mage::app()->getRequest()->getModuleName() == Trio_Blog_Helper_Data::DEFAULT_ROOT;
    }
    ...
    protected function _prepareCollection()
    {
        if (!$this->getData('cached_collection')) {
            ...
            $collection = Mage::getModel('blog/blog')->getCollection()
                    ->addPresentFilter()
                    ->addEnableFilter(Trio_Blog_Model_Status::STATUS_ENABLED)
                    ->addStoreFilter()
                    ->joinComments()

                ...
                if($pInfo->getTypeId() == Mage_Catalog_Model_Product_Type::TYPE_CONFIGURABLE) {
                    ...
                    foreach ($productsCollection as $item) {                        
                            if( Mage::helper('qquoteadv/catalog_product_data')->compareBundles($pID, $superAttribute, $item->getAttribute() ) ){
                              ...
                              $check = $this->checkProdTypeQty($item->getId(), $qtySum['qty'], $type);
...
//                              $check = $this->checkProdTypeQty($params, $params['qty'], $type);
                                $check = $this->checkProdTypeQty($_product, $params['qty'], $type);
    ...
    protected function _initQuoteRequestSave() {
        ...
        if (is_array($paramsProduct) & count($paramsProduct) > 0) {
            foreach ($paramsProduct as $quoteadvProductId => $items) {
                ...
                //preparing items
                if (isset($items['qty']) && is_array($items['qty'])) {
                    foreach ($items['qty'] as $index => $qty) {
					$qty = $qty>0?$qty:1;
                ...
                if (isset($items['client_request'])) {
                    $productsNotes[] = array(
                        ...
                        'client_request' 	=> trim($items['client_request'])
                    );
                }
            }
        }
		...
		$regionId = $paramsAddress['region_id'];
        if($regionId){
            $_regionCollection = Mage::getModel('directory/region')->load($regionId);
            ...
            if($_regionCollection) {
                ...
                $paramsAddress['shipping_id']       = $regionId;                
            }
        }
        ...
        if (isset($paramsAddress['email'])) {
			...
			//EMAIL IS REGESTERED BUT CUSTOMER IS STILL NO LOGINED
            ...
            if ($customerId) {
                ...
                if (isset($paramsAddress['address']) and $paramsAddress['address']!="") {
	                ...
	                if (($billing_address == 'use_default_billing')) {
	                    ...
	                    $paramsAddress['company']   = $primAddress->getData('company');
	                    $paramsAddress['city']      = $primAddress->getData('city');
	                    $paramsAddress['region']    = $primAddress->getData('region');
	
	                    if($primAddress->getData('region_id')) {
	                        $paramsAddress['region_id'] = $primAddress->getData('region_id');
	                    }
	                    
	                    $paramsAddress['address']   = $primAddress->getData('street');
	                    $paramsAddress['postcode']  = $primAddress->getData('postcode');
	                    $paramsAddress['telephone'] = $primAddress->getData('telephone');
	                    $paramsAddress['fax'] = $primAddress->getData('fax');
	                    $paramsAddress['country_id'] = $primAddress->getData('country_id');
	                    
	                    
	                    //#shipping info
	                    if($shippingAddress =  $this->getCustomerSession()->getCustomer()->getPrimaryShippingAddress()) {
	                    
	                        $paramsAddress['shipping_prefix']       = $shippingAddress->getData('prefix');                    
	                        $paramsAddress['shipping_firstname']    = $shippingAddress->getData('firstname');
	                        $paramsAddress['shipping_middlename']   = $shippingAddress->getData('middlename');
	                        $paramsAddress['shipping_lastname']     = $shippingAddress->getData('lastname');
	                        $paramsAddress['shipping_suffix']       = $shippingAddress->getData('suffix');
	
	                        $paramsAddress['shipping_company']  = $shippingAddress->getData('company');
	                        $paramsAddress['shipping_city']     = $shippingAddress->getData('city');
	                        $paramsAddress['shipping_region']   = $shippingAddress->getData('region');
	
	                        if($primAddress->getData('region_id')) {
	                            $paramsAddress['shipping_region_id'] = $shippingAddress->getData('region_id');
	                        }
	
	                        $paramsAddress['shipping_address']      = $shippingAddress->getData('street');
	                        $paramsAddress['shipping_postcode']     = $shippingAddress->getData('postcode');
	                        $paramsAddress['shipping_telephone']    = $shippingAddress->getData('telephone');
	                        $paramsAddress['shipping_fax']          = $shippingAddress->getData('fax');
	                        $paramsAddress['shipping_country_id']   = $shippingAddress->getData('country_id');
	                    }else{
	                        //#shipping info
	                        if(isset($paramsAddress['prefix'])) {       $paramsAddress['shipping_prefix'] = $paramsAddress['prefix']; }
	                        if(isset($paramsAddress['middlename'])) {   $paramsAddress['shipping_middlename']   = $paramsAddress['middlename']; }
	                        if(isset($paramsAddress['suffix'])) {       $paramsAddress['shipping_suffix'] = $paramsAddress['suffix']; }
	
	                        $paramsAddress['shipping_firstname']    = $paramsAddress['firstname'];
	                        $paramsAddress['shipping_lastname']     = $paramsAddress['lastname'];
	
	
	                        $paramsAddress['shipping_company']  = $paramsAddress['company'];
	                        $paramsAddress['shipping_city']     = $paramsAddress['city'];
	                        $paramsAddress['shipping_region']   = $paramsAddress['region'];
	
	                        if(isset($paramsAddress['region_id'])) {    $paramsAddress['shipping_region_id'] =  $paramsAddress['region_id']; }
	                        if(isset($paramsAddress['fax'])) {          $paramsAddress['shipping_fax']          = $paramsAddress['fax']; }
	
	                        $paramsAddress['shipping_address']      = $paramsAddress['address'];
	                        $paramsAddress['shipping_postcode']     = $paramsAddress['postcode'];
	                        $paramsAddress['shipping_telephone']    = $paramsAddress['telephone'];                    
	                        $paramsAddress['shipping_country_id']   = $paramsAddress['country_id']; 
	                    }
        ...
        return array('itemsData' => $itemsData, 'productsNotes' => $productsNotes, 'paramsAddress' => $paramsAddress);
    }
        ...
        if ($quoteId && $this->getRequest()->isPost()) {
            ...
            if (!$welcome) {
			  $this->_redirect("*/*");
                ...
                try {
                    Mage::getModel('qquoteadv/qqadvproduct')->addNotes($data['productsNotes']);
                } catch (Exception $e) {
                ...
                if( $autoProposal && $this->sendAutoProposalEmail() && Mage::helper('qquoteadv')->validLicense('auto-proposal') ) {
    ...
    public function sendEmail($params) {
	    ...
	    $bccData = explode(";", $bcc); 
            ...
            $template2->setSenderName($recipientName);
            $template2->setSenderEmail($recipientEmail );
             * 
             */
        }
        ...
        //if($bcc) $processedTemplate2 = $template2->getProcessedTemplate($vars);
		/*
         * getProcessedTemplate is called inside send()
         */
        $res = $template->send($recipientEmail, $recipientName, $vars);
        //if($bcc) $res2 = $template2->send($bccData, $recipientName, $vars);
    ...
    {       
        ...
        try {
            ...
            $attribute = serialize($params);
            $quoteItem->setAttribute($attribute);
    ...
    public function sendAutoProposalEmail() {
        if(!Mage::helper('qquoteadv')->validLicense('auto-proposal')){
            return false;
        }
         ...
         * getProcessedTemplate is called inside send()
         */
        $res = $template->send($_quoteadv->getEmail(), "name", $vars);
    ...
    public function goToQuoteAction(){
         ...
         $autoConfirm='';
         if(Mage::getStoreConfig('qquoteadv/emails/auto_confirm', $quote->getStoreId()) == 1) {
         ...
         $allowed = Mage::helper('qquoteadv')->validLicense('email-auto-login');
         ...
         if($configured && $allowed && $hash === $origUrlHash){
           ...
           if(!$my && !$autoConfirm){
             ...
             $this->_redirectUrl(Mage::getUrl("*/view/history/"));
         }
    }
